            ...
            if (ctxSemanal) {
                graficoSemanal = new Chart(ctxSemanal, {
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: { position: 'top' },
                            // <<< Aquí van todas las callbacks del tooltip >>>
                            tooltip: {
                                backgroundColor: '#333',
                                titleColor: '#fff',
                                bodyColor: '#fff',
                                callbacks: {
                                    title: items => {
                                        // items es un array, siempre [0]
                                        const it = items[0];
                                        return `${it.label} – ${it.dataset.label}`;
                                    },
                                    label: item => `Porcentaje: ${item.raw}%`,
                                    afterLabel: () => `Total registros: ${graficoSemanal._totalRegistros}`,
                                    afterBody: items => {
                                        const dia = items[0].label;
                                        const lista = graficoSemanal._estandaresPorDia[dia] || [];
                                        return ['Estándares evaluados:', ...lista];
                                    }
                                }
                            },
